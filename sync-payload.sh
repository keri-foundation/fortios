...
fortweb_package_runtime_js_script() {
...
try {
  ...
  if (Object.prototype.hasOwnProperty.call(scripts, "check:runtime-js")) {
    process.stdout.write("check:runtime-js");
    process.exit(0);
  }

  if (Object.prototype.hasOwnProperty.call(scripts, "build:runtime")) {
...
verify_fortweb_runtime_js() {
  ...
  if runtime_js_script="$(fortweb_package_runtime_js_script "${package_json_path}")"; then
    :
  else
    checker_exit_code=$?
    case "${checker_exit_code}" in
      1)
        echo "error: FortWeb checkout appears to use TS runtime sources but does not provide npm run check:runtime-js or npm run build:runtime." 1>&2
        echo "       Update FortWeb or use a compatible ref before local payload sync." 1>&2
        exit 1
        ;;
      2)
  ...
  echo "[sync-payload] verifying FortWeb runtime JavaScript freshness via npm run ${runtime_js_script}"
  (
    cd "${FORTWEB_SOURCE_DIR}"
    npm run "${runtime_js_script}"
  )
}
